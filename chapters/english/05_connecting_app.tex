...
% Improved section text based on MICSBOL/ESP32_BT_Controller repository behavior.
% Notes:
% - Uses \verb|...| for filenames/macros to avoid underscore/math-mode issues with babel+hyperref.
% - Debug flags reflect debug_config.h in the repository.

\section{Install the Application}
Install the RC Controller Android application on your smartphone using the distribution method provided
by your project (APK, store link, or internal deployment).

\section{Enable Bluetooth}
Enable Bluetooth on your phone before launching the app. On newer Android versions, the app may request
additional permissions (for example ``Nearby devices'' or Location-related permissions) in order to scan
for Bluetooth devices. Grant the requested permissions so the device list can be populated correctly.

\section{Connect to the ESP32}
Power the ESP32 and open the RC Controller app. From the list of available Bluetooth devices, select the
ESP32 controller.

Once the connection is established, the control interface becomes active and the application begins sending
control data to the ESP32 (joysticks, knobs, switches, and button events).

\section{Connection Status}
The application displays the connection status to confirm that a Bluetooth serial link is active.

On the firmware side, a connected client is detected using \verb|SerialBT.hasClient()| (used by the telemetry
module). If the phone disconnects, the firmware returns to ``waiting'' state and the application must reconnect
before control and telemetry resume.

\section{Debug Mode (Viewing Bluetooth Communication)}
The firmware includes an optional debug system that prints decoded controller data to the USB Serial Monitor.
This is the fastest way to confirm that:
\begin{itemize}
    \item the phone is connected to the correct ESP32 device,
    \item packets are being received and decoded correctly, and
    \item joystick/knob/switch values change when you move controls in the app.
\end{itemize}
...
Disable debugging by setting:

\begin{verbatim}
    #define DEBUG_ENABLED 0
\end{verbatim}

When \verb|DEBUG_ENABLED| is enabled, the repository also enables the individual debug categories:
...
After changing any debug setting, compile and upload the firmware again to the ESP32.
