...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
        block/.style={rectangle, draw, thick, minimum width=11.8cm, minimum height=1.05cm, align=center},
        arrow/.style={->, thick},
        node distance=0.9cm
        ]
        \node[block] (android) {Android App (UI Controls)};
        \node[block, below=of android] (bt) {Bluetooth Serial (SPP)};
        \node[block, below=of bt] (rx) {\texttt{receiver.cpp} (decode into \texttt{rcStatePacket} / \texttt{rcEventPacket})};
        \node[block, below=of rx] (control) {\texttt{control.cpp} (control policy: mapping inputs to actions)};
        \node[block, below=of control] (hal) {\texttt{hal\_output.cpp} (hardware mechanisms: PWM/GPIO/MCP)};
        \node[block, below=of hal] (hw) {Hardware (motors, servos, LEDs, switches, MCP23017)};

        \draw[arrow] (android) -- (bt);
        \draw[arrow] (bt) -- (rx);
        \draw[arrow] (rx) -- (control);
